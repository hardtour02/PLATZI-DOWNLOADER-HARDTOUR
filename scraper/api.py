...
class AsyncPlatzi:
                    ...
                    if unit.video:
                        ...
                        if subs:
                            for sub in subs:
                                ...
                                dst_sub = CHAP_DIR / f"{file_name}{lang}.vtt"
                        ...
                        if unit.resources:
                            ...
                            if files:
                                for archive in files:
                                    res_name = unquote(os.path.basename(archive))
                                    dst_res = CHAP_DIR / f"{jdx:02}-{res_name}"
                                    Logger.print(f"[{dst_res.name}]", "[DOWNLOADING-FILES]")
                                    await download(archive, dst_res)

                            readings = unit.resources.readings_url
                            if readings:
                                dst_read = CHAP_DIR / f"{jdx:02}-Lecturas recomendadas.txt"
                            ...
                            if summary:
                                dst_sum = CHAP_DIR / f"{jdx:02}-Resumen.html"
                                with open(dst_sum, "w", encoding="utf-8") as f:
                                    f.write(summary)

                    if unit.type == TypeUnit.LECTURE:
                        dst_lec = CHAP_DIR / f"{file_name}.mhtml"
                        await self.save_page(unit.url, path=dst_lec)
                        unit_local_path = str(Path(dst_lec).absolute())

                    if unit.type == TypeUnit.QUIZ:
                        dst_qz = CHAP_DIR / f"{file_name}.mhtml"
                        await self.save_page(unit.url, path=dst_qz)
                        unit_local_path = str(Path(dst_qz).absolute())
    ...
    @try_except_request
    async def save_page(self, src: str | Page, path: str | Path = "source.mhtml", **kwargs):
        overwrite: bool = kwargs.get("overwrite", False)
        if not overwrite and Path(path).exists(): return
        ...
        except Exception: raise Exception("Error saving page as mhtml")
        if isinstance(src, str): await page.close()
    ...
    async def _save_course_assets(self, course_slug: str, metadata: Dict, page: Page):
        try:
            ...
            if not logo_path.exists(): logo_path = course_madre / "logo.jpg"
            if logo_path.exists():
                metadata["logo_url"] = f"assetmadre/{course_slug}/{logo_path.name}"
            else:
                legacy_match = find_asset_match(legacy_badges, course_slug)
                if legacy_match: metadata["logo_url"] = f"assetmadre/badges/{legacy_match.name}"
            ...
            if not thumb_path.exists(): thumb_path = course_madre / "thumbnail.png"
            if thumb_path.exists():
                ...
                if legacy_match: metadata["thumbnail_url"] = f"assetmadre/thumbnails/{legacy_match.name}"
            ...
            if not metadata.get("logo_url"): metadata["logo_url"] = "FALLBACK_LOGO_SVG"
    ...
    async def _save_state(self):
        ...
        except Exception as e: Logger.error(f"Error saving session state: {e}")

    async def _load_state(self):
        if not SESSION_FILE.exists() or SESSION_FILE.stat().st_size == 0: return
        try:
            cookies = read_json(str(SESSION_FILE))
            if cookies: await self.context.add_cookies(cookies)
        except Exception as e: Logger.error(f"Error loading session state: {e}")
