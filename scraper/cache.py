...
from scraper.constants import CACHE_DIR

class Cache:
    @staticmethod
    def _cache_dir() -> Path: return CACHE_DIR

    @classmethod
    def _path(cls, id: str) -> Path: return cls._cache_dir() / f"{id}.pkl"
    ...
    async def set(cls, id: str, content: object) -> None:
        ...
        try:
            ...
            async with aiofiles.open(path, "wb") as file: await file.write(data)
        except: pass

    @classmethod
    def clear(cls):
        if cls._cache_dir().exists(): shutil.rmtree(cls._cache_dir())
    ...
    def cache_async(cls, func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            if cached := await cls.get(cache_id): return cached
